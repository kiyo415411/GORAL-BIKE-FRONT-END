...
export default function BikeModel({ ...props }) {
  ...
  useEffect(() => {
    ...
    if (hovered) {
      document.body.style.cursor = `url('data:image/svg+xml;base64,${window.btoa(
        cursor
      )}'), auto`;
      return () =>
        (document.body.style.cursor = `url('data:image/svg+xml;base64,${window.btoa(
  ...
  useFrame(() => (frontWheel.current.rotation.x += 0.012));
  useFrame(() => (backWheel.current.rotation.x += 0.012));
